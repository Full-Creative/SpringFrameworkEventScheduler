package com.eventschedule.datastore;
...
import com.eventschedule.helper.SerializerHelper;
...
//		ofy().save().entity(event).now();
	...
	public Event updateEvent(Event event) {
		...
		try {
			eventEntity = datastore.get(KeyFactory.createKey("Event", event.getEventID()));
			if (eventEntity != null) {
	...
	public boolean createParticipant(ParticipantDetails participant) {
		if (participant.getName() == null || participant.getName().equals("") || participant.getTimeZone() == null
				|| participant.getTimeZone().equals(""))
			return false;
		String id = Generators.timeBasedGenerator().generate().toString();
	...
	public boolean addParticipant(ParticipantDetails participant) {
		...
		try {
			...
			if (eventEntity.getProperty("ParticipantKey") != null) {
				participantIDinEvent = (List<String>) serializer
						.propertyToObject(eventEntity.getProperty("ParticipantKey").toString(), ArrayList.class);
	...
	public Event entityToObject(Entity entity) {
		...
		if (entity.getProperty("ParticipantKey") != null) {
			keySet = (Set<String>) serializer.propertyToObject(entity.getProperty("ParticipantKey").toString(),
					HashSet.class);
...
//		Entity participant = datastore.get(participantKey);
		...
		for (Entity e : pq.asList(FetchOptions.Builder.withDefaults())) {
			...
			/*
			 * if (participantTimeZone != null) { Calendar convertedTime =
			 * Calendar.getInstance();
			 * 
			 * SimpleDateFormat sdf = new SimpleDateFormat("dd-M-yyyy HH:mm:ss");
			 * sdf.setTimeZone(zoneID); convertedTime.setTimeZone(zoneID);
			 * 
			 * String res = sdf.format(event.getEventTime());
			 * convertedTime.setTime(sdf.parse(res));
			 * event.setEventTime(convertedTime.getTimeInMillis()); System.out.println(res);
